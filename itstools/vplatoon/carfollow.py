...
class PlatoonFollowLaw(RegularVehicle):
    """
    ...
    def s_d(self) -> float:
        ...
        """
        try:
            return max(self.s0 + 1 / (self.w * self.k_x) * self.v_t, self.s_ref)
        except TypeError:
    ...
    def e_v(self) -> float:
        ...
        """
        if self.s_ref:
            return self.v_t - self.vd
        else:
            raise NameError("Not reference registered for ACC")
    ...
    def e_s(self) -> float:
        ...
        """
        if self.s_ref:
            return self.s - self.s_d
        else:
            raise NameError("Not reference registered for CACC")
    ...
    def register_control_space(self, control):
        """
            This registers an external control signal into the vehicle behavior
        """
        self.sd = control
    ...
    def register_reference_speed(self, reference):
        """
            This registers an external reference signal into the vehicle behavior for speed behavior (ACC)
        """
        self.v_ref = reference
        self.acc = True

    def register_reference_space(self, reference):
        """
...
class VehiclePlatoon(PlatoonFollowLaw):
    """
    ...
    def set_parameters(self, **kwargs) -> None:
        ...
        """
        self._A = kwargs.get("A", AREA)
        self._M = kwargs.get("M", MASS)
        self._CDMAX = kwargs.get("CD", C_DMAX)
    ...
    def C_d(self) -> float:
        """ 
            Drag coefficient
        """
        return self.Cdmax * 1 / (1 + sqrt(exp(self.s * -1)))


    def u_control(self) -> float:
        """ 
            Control call for ACC or CACC 
        """
        if self.acc:
            return self.control(self.e_v)
        elif self.cacc:
            return self.control(self.e_s)
        else:
            return 0
...
class OpenLoop(CarFollowLaw):
    ...
    def set_parameters(self, t_e=T_E) -> None:
        """
            Set default parameters
        """
        self.te = t_e
    ...
    @te.setter
    def te(self, value: float = T_E) -> None:
        self._te = value
